...
const SettingsModal: React.FC<SettingsModalProps> = ({ onClose }) => {
  const modalStyles = {
    ...
    modalContent: "bg-white rounded-lg p-8 relative w-11/12 md:w-96 h-full md:h-auto overflow-y-auto", // Adjust width and height for mobile and larger screens
      ...
      <div className={modalStyles.modalContent}>
        ...
        <ul>
          {settings.map((setting, index) => (
            <li key={index} className="py-2 border-t border-b flex items-center justify-between">
        ...
        <button className={modalStyles.logoutButton}>Logout</button>
      </div>
    </div>
  );
};
